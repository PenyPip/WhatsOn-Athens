...
function screeningEvent(st: StrapiShowtime, movie: StrapiMovie, movieUrl: string): JsonLdObject | null {
...
export function buildMovieDetailJsonLd(input: MovieDetailJsonLdInput): JsonLdObject {
  const { movie, slug, genreLabel, showtimes } = input;
  const tl = movieTitleLines(movie);
  ...
  const screenings = showtimes
    .map((st) => screeningEvent(st, movie, movieUrl))
