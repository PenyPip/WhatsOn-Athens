...
/** Slug σελίδας More από URL (π.χ. /tickets/cinemas/…, /tickets/cinema/summer/…, /tickets/theater/…). */
function morePageSlugFromUrl(url) {
  const s = String(url || '').trim();
  const pathOnly = s.replace(/^https?:\/\/[^/]+/i, '').split(/[?#]/)[0];
  const m =
    pathOnly.match(/\/tickets\/cinemas?\/(?:summer\/)?([^/]+)/i) ||
    pathOnly.match(/\/tickets\/cine-([^/?#]+)/i) ||
    pathOnly.match(/\/tickets\/theater\/(?:summer\/)?([^/]+)/i);
  if (!m) return '';
  try {
...
function isMoreBoilerplateTitlePart(part) {
  const s = String(part || '').trim();
  if (!s) return true;
  return /^(εισιτήρια|tickets|more\.com|online!?|viva)/i.test(s);
}

function cleanMoreDisplayTitle(raw) {
  let s = decodeHtmlEntities(String(raw || '')).trim();
  if (!s) return '';
  s = s.replace(/\s*[|·•]\s*Εισιτήρια.*$/iu, '');
  s = s.replace(/\s*[|·•]\s*Tickets.*$/iu, '');
  s = s.replace(/\s*[|·•]\s*More\.com.*$/iu, '');
  s = s
    .split(/\s*[|·•]\s*/)
    .map((p) => p.trim())
...
function hasGreekLetters(s) {
  return /[\u0370-\u03FF\u1F00-\u1FFF]/.test(String(s || ''));
}

function isMostlyLatinTitle(s) {
  const t = String(s || '').trim();
  if (!t) return false;
  const letters = t.replace(/[^a-zA-Z\u0370-\u03FF\u1F00-\u1FFF]/g, '');
  if (!letters) return false;
  const latin = (letters.match(/[a-zA-Z]/g) || []).length;
  return latin / letters.length >= 0.6;
}

function humanizeMorePathSlug(slug) {
  const s = String(slug || '')
    .trim()
    .toLowerCase()
    .replace(/[_]+/g, '-')
...
function absoluteMoreUrl(raw) {
  let u = decodeHtmlEntities(String(raw || '').trim());
  if (!u) return '';
  if (u.startsWith('//')) u = `https:${u}`;
  if (u.startsWith('/')) u = `https://www.more.com${u}`;
  if (/^www\.more\.com\//i.test(u)) u = `https://${u}`;
  return u;
}
...
function parseMoreTitleParts(raw, { pathSlug = '', playTitle = '' } = {}) {
  ...
  const displayTitle = play || cleaned;
  const parts = cleaned
    .split(/\s*[|·•]\s*/)
    .map((p) => p.trim())
    .filter((p) => p && !isMoreBoilerplateTitlePart(p));
  ...
  if (!originalTitle && play && isMostlyLatinTitle(play)) {
    originalTitle = play;
  }
  ...
  if (!originalTitle) originalTitle = displayTitle || cleaned;

  return {
    title: displayTitle || cleaned,
    originalTitle: originalTitle || displayTitle || cleaned,
  };
}

function pickHtmlById(html, id) {
  const re = new RegExp(
    `id="${id.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}"[^>]*>([\\s\\S]*?)</`,
    'i',
  );
...
function extractMoreLdEvent(html) {
  for (const m of String(html || '').matchAll(
    ...
    try {
      ...
      for (const node of nodes) {
        if (node && (node['@type'] === 'Event' || node['@type'] === 'Movie')) return node;
      }
    } catch {
      /* ignore bad json-ld */
    }
...
function extractMoreCastNames(html) {
  const names = [];
  const seen = new Set();
  const push = (raw) => {
    const n = stripHtmlToText(raw);
    if (!n || n.length < 2 || n.length > 80) return;
    if (/^(πρωταγωνιστές|ηθοποιοί|συντελεστές|cast)$/i.test(n)) return;
  ...
  const castSection = html.match(
    /id="PageContent_PlayDetails_castSection"[\s\S]*?<\/section>/i,
  )?.[0];
  if (castSection) {
    for (const m of castSection.matchAll(/<p class="name">([^<]+)/gi)) push(m[1]);
    if (!names.length) {
      for (const m of castSection.matchAll(/class="[^"]*name[^"]*"[^>]*>([^<]+)/gi)) {
        push(m[1]);
      }
    }
  }
...
function normalizeMoreAttachmentUrl(raw) {
  const abs = absoluteMoreUrl(raw);
  if (!abs) return '';
  // Πάντα με trailing slash — το More κάνει 301 χωρίς αυτό.
  const m = abs.match(/^(https?:\/\/www\.more\.com\/getattachment\/[0-9a-f-]{36})(?:\/.*)?$/i);
  if (m) return `${m[1]}/`;
  return abs;
}

async function scrapeMorePageMeta(moreUrl) {
  const url = String(moreUrl || '').trim();
  if (!url || !/^https?:\/\//i.test(url)) return null;
  try {
    const html = await fetchText(url);
    const pick = (re) => html.match(re)?.[1]?.trim() || '';
    const ld = extractMoreLdEvent(html);
    const pathSlug = morePageSlugFromUrl(url);

    const playTitle =
      pickHtmlById(html, 'playTitle') ||
      pickHtmlById(html, 'TitleLabel') ||
      stripHtmlToText(ld?.name || '');

    const ogTitle =
      pick(/property="og:title"\s+content="([^"]+)"/i) ||
      pick(/<title[^>]*>([^<]+)<\/title>/i);
    ...
    const pushId = (id) => {
      ...
      if (!/^[0-9a-f-]{36}$/.test(key) || seenIds.has(key)) return;
      seenIds.add(key);
    ...
    for (const m of html.matchAll(/\/getattachment\/([0-9a-f-]{36})/gi)) {
      pushId(m[1]);
    }

    const posterCandidates = [];
    const pushPoster = (raw) => {
      const u = normalizeMoreAttachmentUrl(raw);
      if (!u || looksLikeGenericMoreLogo(u)) return;
      if (posterCandidates.includes(u)) return;
      posterCandidates.push(u);
    };
...
function looksLikeGenericMoreLogo(url) {
  const u = String(url || '').toLowerCase();
  if (!u) return true;
  if (/\/site\/images\/logos\//.test(u)) return true;
  if (/more\.svg|favicon|apple-touch/.test(u)) return true;
  if (/\/icons\/cinemas\/no[_-]?image/i.test(u)) return true;
  return false;
}

function looksLikeWeakMoreSocialImage(url) {
  const u = String(url || '').toLowerCase();
  return /viva-tickets\.png/i.test(u);
}

/**
 * Κατεβάζει εικόνα από More και την ανεβάζει στο Strapi Media Library.
 * @returns {Promise<number|null>} file id
 */
async function uploadRemotePosterToStrapi(strapi, imageUrl, nameHint = 'poster') {
  const url = normalizeMoreAttachmentUrl(imageUrl) || String(imageUrl || '').trim();
  if (!url || !/^https?:\/\//i.test(url) || looksLikeGenericMoreLogo(url)) return null;
  ...
  try {
    ...
    const uploaded = await strapi.plugin('upload').service('upload').upload({
      data: {
        fileInfo: {
          name: fileName,
          alternativeText: nameHint || fileName,
          caption: 'Από More.com (αυτόματη εισαγωγή)',
        },
      },
      files: {
        path: tmp,
    ...
    return file?.id != null ? Number(file.id) : null;
  } catch (e) {
    strapi.log.warn(`[more-lookup] poster upload απέτυχε (${url}): ${e?.message || e}`);
    return null;
  } finally {
    clearTimeout(timer);
    if (tmp) {
...
function minutesBetweenMoreDates(startRaw, endRaw) {
  const start = new Date(String(startRaw || '').trim());
  const end = new Date(String(endRaw || '').trim());
  if (Number.isNaN(start.getTime()) || Number.isNaN(end.getTime())) return null;
  const mins = Math.round((end.getTime() - start.getTime()) / 60000);
  if (!Number.isFinite(mins) || mins < 20 || mins > 400) return null;
  return mins;
}

function pickLikelyDurationMinutes(candidates) {
  const values = (candidates || []).filter((n) => Number.isFinite(n) && n >= 20 && n <= 400);
  if (!values.length) return null;
  const counts = new Map();
  for (const v of values) counts.set(v, (counts.get(v) || 0) + 1);
  // Αγνόησε το συχνό default 60 αν υπάρχουν άλλες τιμές.
  const nonDefault = values.filter((v) => v !== 60);
  const pool = nonDefault.length >= Math.max(2, Math.ceil(values.length * 0.25)) ? nonDefault : values;
  const sorted = [...pool].sort((a, b) => a - b);
  const median = sorted[Math.floor(sorted.length / 2)];
  let best = median;
  let bestCount = -1;
  for (const [v, c] of counts.entries()) {
    if (pool === nonDefault && v === 60 && nonDefault.length) continue;
    if (c > bestCount || (c === bestCount && Math.abs(v - median) < Math.abs(best - median))) {
      best = v;
      bestCount = c;
    }
  }
...
async function enrichFromMoreEvents(code) {
  const events = await fetchMoreEventsByGroupCodeSafe(code);
  if (!events.length) return { duration: null, runStart: null, runEnd: null, venueNames: [], eventCount: 0 };

  const durationCandidates = [];
  const dates = [];
  const venueNames = new Set();
  for (const ev of events) {
    const fromField = Number(ev.duration);
    if (Number.isFinite(fromField) && fromField > 0) durationCandidates.push(fromField);
    const fromDates = minutesBetweenMoreDates(ev.eventDate, ev.eventEndDate);
    if (fromDates != null) durationCandidates.push(fromDates);
    const dt = String(ev.eventDate || '').trim();
    if (dt) {
      const d = new Date(dt);
      if (!Number.isNaN(d.getTime())) dates.push(d);
    }
    const vn = String(ev.venueName || '').trim();
    if (vn) venueNames.add(vn);
  }
  dates.sort((a, b) => a - b);
  const toDate = (d) => {
    if (!d) return null;
  ...
  return {
    duration: pickLikelyDurationMinutes(durationCandidates),
    runStart: toDate(dates[0]),
    runEnd: toDate(dates[dates.length - 1]),
    venueNames: [...venueNames],
    eventCount: events.length,
  };
}

async function fetchMoreEventsByGroupCodeSafe(code) {
  try {
    const { fetchMoreEventsByGroupCode } = require('./moreApi');
    return await fetchMoreEventsByGroupCode(code);
  } catch {
    return [];
  }
}

async function uniqueContentSlug(strapi, uid, baseSlug) {
  const base = String(baseSlug || 'item')
    .toLowerCase()
    .replace(/[^a-z0-9-]+/g, '-')
    .replace(/^-+|-+$/g, '')
    .slice(0, 80) || 'item';
 ...
 */
async function createCmsContentFromMoreCatalog(strapi, options = {}) {
  const code = String(options.eventGroupCode || '').trim();
  if (!code) throw new Error('Απαιτείται eventGroupCode');

  const kind = String(options.kind || options.catalogKind || 'movie').trim();
  const isTheater = kind === 'show' || kind === 'theater_show' || options.category === 'theater';
  const contentType = isTheater ? 'theater_show' : 'movie';
  ...
  const enriched = {
    ...
    eventCount: eventsMeta.eventCount,
    posterUrl: pageMeta?.image || null,
    posterUploaded: false,
    originalTitle: pageMeta?.originalTitle || titleParts.originalTitle || null,
  };

  const posterCandidates = [
    ...(pageMeta?.posterCandidates || []),
    pageMeta?.image,
    ...(pageMeta?.attachmentUrls || []),
  ].filter((u, i, arr) => u && arr.indexOf(u) === i && !looksLikeGenericMoreLogo(u));

  // Social og crops τελευταία.
  posterCandidates.sort((a, b) => Number(looksLikeWeakMoreSocialImage(a)) - Number(looksLikeWeakMoreSocialImage(b)));

  let posterId = null;
  for (const candidate of posterCandidates.slice(0, 6)) {
    posterId = await uploadRemotePosterToStrapi(strapi, candidate, pathSlug || title);
    if (posterId) {
      enriched.posterUrl = candidate;
      enriched.posterUploaded = true;
      break;
    }
  }

  if (isTheater) {
    const slug = await uniqueContentSlug(strapi, uid, pathSlug);
    const data = {
      title,
      slug,
      synopsis: enriched.synopsis || undefined,
      director: enriched.director || undefined,
      duration: enriched.duration || undefined,
      run_start: enriched.runStart || undefined,
      run_end: enriched.runEnd || undefined,
      more_link: moreUrl || undefined,
    ...
    return {
      ok: true,
      contentType,
      entry: { id: created.id, title: created.title, slug: created.slug, posterId },
      enriched,
      message:
        `Δημιουργήθηκε draft παράσταση «${created.title}» (#${created.id}) — unpublished` +
        (posterId ? ' · με αφίσα' : ''),
    };
  }

  const originalTitle = await uniqueMovieOriginalTitle(
    strapi,
    String(
      options.originalTitle ||
        pageMeta?.originalTitle ||
        titleParts.originalTitle ||
        title,
    ).trim() || title,
  );
  enriched.originalTitle = originalTitle;

  const slug = await uniqueContentSlug(
    strapi,
    uid,
    slugifyVenueNameForLookup(originalTitle) || pathSlug,
  );
  const data = {
    title,
  ...
  return {
    ...
    enriched,
    message:
      `Δημιουργήθηκε draft ταινία «${created.title}» (#${created.id}) — unpublished` +
      (posterId ? ' · με αφίσα' : ''),
  };
}
